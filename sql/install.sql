...
-- Trigger to set the price_paid field in the ticket table based on ticket_pricing
CREATE TRIGGER ticket_price
BEFORE INSERT ON ticket
FOR EACH ROW
BEGIN
    DECLARE default_price DECIMAL(5,2);
    SELECT price INTO default_price
    FROM ticket_pricing
    WHERE event_id = NEW.event_id AND category_id = NEW.category_id;
    IF default_price IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Price not found for the given event and category.';
    ELSE
        SET NEW.price_paid = default_price;
    END IF;
END;
//

-- Trigger to prevent price update after purchase
CREATE TRIGGER prevent_price_update
BEFORE UPDATE ON ticket
FOR EACH ROW
BEGIN
    IF NEW.price_paid != OLD.price_paid THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Price cannot be updated after purchase.';
    END IF;
END;
//

-- Trigger to prevent resale if ticket is used
CREATE TRIGGER prevent_resale_if_used
BEFORE INSERT ON resale_offer
FOR EACH ROW
BEGIN
    DECLARE is_used BOOLEAN;
    SELECT is_activated INTO is_used
    FROM ticket
    WHERE ticket_id = NEW.ticket_id;

    IF is_used THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Ticket cannot be resold if it has been used.';
    END IF;
END;
//

-- Trigger to enforce ownership transfer on resale transaction
CREATE TRIGGER enforce_ownership_transfer
AFTER INSERT ON resale_transaction
FOR EACH ROW
BEGIN
    UPDATE ticket
...
-- Trigger to update BOOLEAN fields in resale_offer and resale_request and resale_transaction
CREATE TRIGGER after_resale_transaction
AFTER INSERT ON resale_transaction
FOR EACH ROW
BEGIN
